...
-- ── Avis : lecture publique, ÉCRITURE RÉSERVÉE À L'EDGE FUNCTION ──
-- Pas de policy INSERT pour `authenticated` : submit-review vérifie
-- l'éligibilité (commande livrée / projet terminé, pas de doublon) et
-- recalcule la moyenne du produit/artisan/designer AVANT d'insérer, via le
-- client service role qui bypasse RLS. Un INSERT direct autorisé ici
-- court-circuiterait ces deux contrôles — même logique que le wallet.
